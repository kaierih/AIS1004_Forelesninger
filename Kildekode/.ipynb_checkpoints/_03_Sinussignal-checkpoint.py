...
# Demo 4:
# Sinusbølger representert med vektor       
class PhasorDemo():
    def __init__(self, fig_num=1, fig_size=(9, 4)):
        # Set up canvas
        plt.close(fig_num)
        ...
        self.SineWaves = timeSeriesPlot(ax1, self.t, A_max = 1.1,  N = 1)
        
        self.SineWaves.setStyles([{'color': 'tab:blue'}])
        
        self.SineWaves.setLabels([r'$x(t) = A\cdot \cos(2\pi t + \phi)$'])

        
        # Set up vector subplot
        ax2 = plt.subplot(1, 5, (4,5))
        ax2.set_title(r"Kompleks amplitude $\boldsymbol{A} = A\cdot e^{j\phi_k}$")
        ax2.set_aspect(1)
        
        self.VectorSumPlot = vectorPlot(ax2, A_max = 1.1, N = 1)
        
        self.VectorSumPlot.setStyles([{'color': 'tab:blue'}])
    ...
    def update(self, **kwargs):
        ...
        self.VectorSumPlot.update([v1_x], [v1_y])        
# Demo 5: 
# Vektorrepresentasjon av komplekst tall på polarform        
class demo5:
